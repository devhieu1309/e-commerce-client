...
function FromRegister() {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      console.log("Google OAuth URL:", data);

      if (data.url) {
        window.location.href = data.url; // chuyển hướng sang trang đăng nhập Google
      } else {
        alert("Không lấy được link Google OAuth!");
      }
    } catch (error) {
      ...
      <div className=" flex items-center justify-center mt-5 mb-5 font-sans">
        ...
        <div className="bg-white shadow-lg rounded-lg w-96 p-6 border border-gray-100">
          ...
          <form onSubmit={handleSubmit}>
            ...
            />
            {errors.phone && (
              <p className="text-red-600 text-sm mb-2">{errors.phone[0]}</p>
            )}
            <input
              type="password"
              name="password"
          ...
          <div className="flex justify-center space-x-2">
            ...
            <button
              ...
              className="flex items-center bg-[#db4437] text-white px-4 py-2 rounded-md text-sm"
            >
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="w-4 h-4 mr-2"
